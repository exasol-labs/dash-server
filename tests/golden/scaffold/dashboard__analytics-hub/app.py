...
BUSINESS_CAPTION = 'Replace the business SQL files with schema-specific queries after discovery.'
BUSINESS_SUMMARY_HEADING = 'Business KPI Snapshot'
BUSINESS_CHART_HEADING = 'Business Trend'
BUSINESS_TABLE_HEADING = 'Business Detail'
...
def create_dash_app(server, url_base_pathname, metadata):
    app = Dash(
        ...
        title=metadata.get("title", 'Golden Analytics-Hub'),
    )
    app.layout = html.Div(
        [
            dcc.Interval(id="refresh", interval=15000, n_intervals=0),
            html.H1(metadata.get("title", 'Golden Analytics-Hub')),
    ...
    def refresh_dashboard(_n_intervals):
        ...
        if monitor_rows and not has_error(monitor_rows):
        ...
        elif has_error(monitor_rows):
            # PS26-BUG-012: this used to discard the real `_error` text for a generic
            # "No monitor data available." message, indistinguishable from the query
            # legitimately returning zero rows - an operator watching System Health
            # would never learn Exasol actually failed.
            monitor_figure = _empty_figure(
                f"Exasol query failed for queries/system/monitor.sql: {monitor_rows[0]['_error']}"
            )
        else:
            monitor_figure = _empty_figure("No monitor data available.")

        usage_figure = go.Figure()
        if usage_rows and not has_error(usage_rows):
        ...
        elif has_error(usage_rows):
            usage_figure = _empty_figure(
                f"Exasol query failed for queries/system/usage.sql: {usage_rows[0]['_error']}"
            )
        else:
            usage_figure = _empty_figure("No usage data available.")

        business_figure = go.Figure()
        if business_trend and not has_error(business_trend):
        ...
        if sql_rows and not has_error(sql_rows):
        ...
        if business_summary and has_error(business_summary):
