...
- **`app_export_create`** — Queue a governed export from a registered output on the current live revision. Accepts any format the consumption contract supports (csv, xlsx for dataset outputs; pdf, png, pptx for view outputs), but only dataset formats are currently executable - a view output's formats are structurally valid here yet always fail with consumption_format_unavailable (reason renderer_not_available) until view rendering ships. Check app_outputs_list's per-format policy.format_availability before queuing to see which formats are actually executable right now.
- **`app_exports_admin_list`** — List every principal's export jobs for one app with redacted parameter summaries. Requires the dashboard.manage_consumption capability.
